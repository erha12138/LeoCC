...
在每个 reconfiguration 周期内画出「前一个包与后一个包的时延差值」的散点图。
横轴：周期内时间（取后一个包的时间）
纵轴：delta_delay = delay[i+1] - delay[i]（ms）
...
def compute_delay_deltas(
    times_s: List[float], delays_ms: List[float]
) -> Tuple[List[float], List[float]]:
    """
    计算相邻包之间的时延差值：delta[i] = delay[i+1] - delay[i]。
    横坐标取后一个包的时间 times_s[i+1]。
    返回 (times_for_delta, delta_delays_ms)。
    """
    if len(delays_ms) < 2:
        return [], []

    times_out: List[float] = []
    deltas_out: List[float] = []

    for i in range(len(delays_ms) - 1):
        delta = delays_ms[i + 1] - delays_ms[i]
        times_out.append(times_s[i + 1])
        deltas_out.append(delta)

    return times_out, deltas_out


def plot_trace_delay_deltas(trace_dir: str, trace_id: str) -> None:
    """
    对单个 trace，在每个 reconfiguration 周期内画出「前后相邻包时延差值」的散点图。
    """
    ...
    for period_idx in sorted(periods.keys()):
        ...
        if len(delays_ms) < 2:
            print(f"  [skip] period {period_idx}: need at least 2 samples for delta")
            continue

        times_delta, deltas_ms = compute_delay_deltas(times_s, delays_ms)
        if not times_delta:
            continue

        fig, ax = plt.subplots(figsize=(8, 4))
        ax.scatter(times_delta, deltas_ms, s=10, color="tab:green", alpha=0.7)
        ax.axhline(y=0, color="gray", linestyle="--", linewidth=0.8)

        ax.set_xlabel("Time within reconfiguration period (s)")
        ax.set_ylabel("Delay delta (ms)\n(delay[i+1] - delay[i])")
        ax.set_title(
            f"Trace {trace_id} - Delay delta between consecutive samples, period {period_idx}\n"
        ...
        out_path = os.path.join(
            trace_dir,
            f"trace_{trace_id}_delay_delta_reconfig_period_{period_idx:02d}.png",
        )
        fig.tight_layout()
...
def main() -> None:
    ...
    for name in subdirs:
        ...
        trace_id = name
        plot_trace_delay_deltas(trace_dir, trace_id)
