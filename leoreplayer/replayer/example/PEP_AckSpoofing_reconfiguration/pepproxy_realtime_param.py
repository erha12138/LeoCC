...
OFFSET_MS = 10240 # 从 traces 文件中读取的 offset_ms 值，单位为毫秒

# === QUICKACK（用于 reconfiguration 窗口内加速 ACK）===
# 说明：
# - TCP_QUICKACK 是 Linux TCP 的一个 hint：倾向于立即 ACK（减少 delayed ACK）
# - 该选项可能不是“永久生效”，因此我们在窗口内会周期性重复设置为 1
QUICKACK_PERIOD_MS = 15_000
QUICKACK_WINDOW_MS = 200
QUICKACK_SET_INTERVAL_S = 0.01  # 窗口内每 10ms 触发一次 setsockopt(1)
TCP_QUICKACK_OPT = getattr(socket, "TCP_QUICKACK", None)
...
class PipeContext:
    ...
    def __init__(self, conn_id: int, initial_rate_bps: int, app_buf_bytes: int, queue: "ByteQueue", bucket: "TokenBucket", log_file_path: str = None, offset_ms: int = 12000):
        ...
        # quickack runtime state (avoid too frequent setsockopt)
        self._quickack_enabled = False
        self._quickack_last_set = 0.0
        if log_file_path:
            try:
...
def _in_quickack_window(ctx: PipeContext, now: float) -> bool:
    """
    Whether we are within the periodic [offset_ms, offset_ms+200ms) window.
    Time base: ctx.created_at (monotonic).
    """
    if QUICKACK_PERIOD_MS <= 0 or QUICKACK_WINDOW_MS <= 0:
        return False
    # normalize offset into one period
    offset_ms = int(ctx.offset_ms) % QUICKACK_PERIOD_MS
    elapsed_ms = int(((now - ctx.created_at) * 1000.0)) % QUICKACK_PERIOD_MS
    end_ms = offset_ms + QUICKACK_WINDOW_MS
    if end_ms <= QUICKACK_PERIOD_MS:
        return offset_ms <= elapsed_ms < end_ms
    # window wraps around the period end
    return elapsed_ms >= offset_ms or elapsed_ms < (end_ms % QUICKACK_PERIOD_MS)


def _maybe_set_quickack(sock_in: socket.socket, ctx: PipeContext, now: float):
    """
    Enable QUICKACK for inner->PEP receiving socket during the reconfiguration window.
    In-window: periodically sets TCP_QUICKACK=1 (Linux may treat it as a hint).
    Out-of-window: best-effort sets TCP_QUICKACK=0 once when leaving window.
    """
    if TCP_QUICKACK_OPT is None:
        return

    in_window = _in_quickack_window(ctx, now)
    if in_window:
        # Periodically "refresh" QUICKACK=1 during the window
        if (now - ctx._quickack_last_set) >= QUICKACK_SET_INTERVAL_S:
            try:
                sock_in.setsockopt(socket.IPPROTO_TCP, TCP_QUICKACK_OPT, 1)
                ctx._quickack_enabled = True
            except OSError:
                # Ignore if kernel/socket doesn't allow it in current state
                pass
            ctx._quickack_last_set = now
        return

    # out of window
    if ctx._quickack_enabled:
        try:
            sock_in.setsockopt(socket.IPPROTO_TCP, TCP_QUICKACK_OPT, 0)
        except OSError:
            pass
        ctx._quickack_enabled = False


def _reader(sock_in: socket.socket, q: ByteQueue, ctx: PipeContext):
    try:
        while True:
            # reconfiguration: within each 15s period, enable QUICKACK for 200ms after offset_ms
            _maybe_set_quickack(sock_in, ctx, time.monotonic())
...
def start_pep():
    ...
    while True:
        try:
            ...
            _pipe_bidirectional(sender_conn, client_conn, rate_bps=rate_bps, app_buf_bytes=app_buf_bytes, conn_id=connection_counter, DATA_DIR=DATA_DIR, offset_ms=offset_ms)
